...
```{r}
# Carregar o pacote tools4uplift, se necessário
if (!require(tools4uplift)) {
...
```{r}
barplot(performance_inter_uplift_bogo, col="blue4")
```

```{r}
barplot(performance_inter_uplift_discount, col="red4")
```

```{r, include=FALSE}
#>>>>>>>>>>Checando - Remover depois<<<<<<<<<<
BinUplift(
  data = uplift_treinamento_bogo,
  treat = "treat",
  outcome = "conversion",
  x = "recency",
  n.split = 100,
...
- aprofundar análise comparativa entre campanhas, explicar diferenças entre os barplots, comportamento do uplift incremental do discount

- rodar um terceiro modelo para comparação QINI-based uplift (qLHS) ?

- aplicar "univariate quantization"?
